...
% V6 comment
% TODO Avabar V6
We expect streams to prove advantageous when we have a larger amount of data to transfer between the host and the device. To prove this hypothesis we increase the grid size to 2000x2000 and compare the different versions, showing the results in

Another situation in that streams may be helpful is we want the output in intermediate steps, allowing the communication to overlap with the computation of the next step. So, once again, we rerun the different version with \texttt{outputEvery = 1000} to compare the execution time. The results can be seen in 
