...
int main()
{
    ...
    for (int i = 0; i < NUM_ITERATIONS; i++)
    {
        ...
        for (int s = 0; s < nStreams; s++)
        {
            ...
            cudaStreamCreate(&streamRecive[s]);
        }
            ...
            for (int ystream = 0; ystream < STREAMCOUNT_Y; ystream++)
            {
                ...
                for (int xstream = 0; xstream < STREAMCOUNT_X; xstream++)
                {
                    ...
                    for (int cy = 0; cy < streamSizeY + 2; cy++)
                    {
                        ...
                        cudaMemcpyAsync(&d_Tn[offset], &h_Tn[offset],
                                        streamSizeX * sizeof(float), cudaMemcpyHostToDevice,
                                        stream[streamNr]);
                        cudaMemcpyAsync(&h_Tnp1[offset], &d_Tnp1[offset], streamSizeX * sizeof(float), cudaMemcpyDeviceToHost,
